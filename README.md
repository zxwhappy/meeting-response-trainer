...
- 当前后台入口为“设置 -> 服务内容声明 -> 用户隐私保护指引”；录音类型未声明时常见错误为 `errno: 112`，系统不会弹出授权，小程序设置页也不会出现麦克风项。保存后按官方常见错误说明预留约 5 分钟生效时间，再重新生成预览包。
...
本仓库不能代替所有者完成公众平台隐私声明，也不保存服务端密钥。因此“真实 ASR/AI 联调、iPhone 真机、Android 真机、审核版体验”仍是发布前人工闸门，不应把源码和 Node 测试通过表述为真机已通过。完整清单见 [docs/test-checklist.md](docs/test-checklist.md)。
