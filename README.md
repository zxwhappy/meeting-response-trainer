...
一个原生微信小程序单页练习闭环：用户听完模拟会议发言，准备 30 秒并录制回应，获得“接住对方 / 说出判断 / 推进下一步”三点短反馈，再针对同一场景复练一次并查看前后变化。

当前源码、两组云函数、6 段中文场景 MP3、自动测试和部署文档均已包含在仓库中。代码默认关闭模拟模式，但仓库没有真实 AppID、云环境 ID 或第三方密钥，因此必须先完成下面的人工配置，才能跑通真实 ASR/AI 链路。
...
`npm run check` 会检查 JavaScript/JSON 语法、6 个场景的客户端/服务端一致性、MP3 是否为 16kHz 单声道且不是小占位文件、WXML 事件处理器，以及前端是否疑似含密钥。
